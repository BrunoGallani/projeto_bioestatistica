...
\includegraphics{Projeto_em_grupo_versao-3--pdf-_files/figure-latex/unnamed-chunk-2-1.pdf}
...
\includegraphics{Projeto_em_grupo_versao-3--pdf-_files/figure-latex/unnamed-chunk-2-2.pdf}
...
Pelo uso do teste não paramétrico (\emph{bootstrap}), obtemos as mesmas
conclusões em relação àquelas obtidas pelo uso inadequado do
\texttt{teste\ t} (sem validação da premissa de normalidade):
...
\tightlist
\item
  Rejeição da hipótese nula, uma vez que o intervalo de confiança 95\%
  da média amostral \([0.506, 0.628]\) não compreende o valor esperado
  pela hipótese nula (\(0.16\)).
\end{itemize}
...
\CommentTok{\# Teste de hipótese de duas amostras não paramétrico}
\CommentTok{\# Como não foi feita a validação de premissas de normalidade em nenhuma}
\CommentTok{\# das amostras, faremos o uso do teste não paramétrico de permutação}

\CommentTok{\# H0: mu(somaTestes condicao controle) = mu(somaTestes condicao experimental)}
\CommentTok{\# Ha: mu(somaTestes condicao controle) != mu(somaTestes condicao experimental)}

\CommentTok{\# Dados transformados para o teste t da RQ3 {-}{-} permutação}
\NormalTok{dados\_teste\_t\_perm\_RQ3 }\OtherTok{=} \FunctionTok{data.frame}\NormalTok{(}
    \AttributeTok{id =}\NormalTok{ dados}\SpecialCharTok{$}\NormalTok{DM08\_01,}
    \AttributeTok{score\_teste\_experimental\_controlado =}\NormalTok{ dados}\SpecialCharTok{$}\NormalTok{test.i }\SpecialCharTok{+}\NormalTok{ dados}\SpecialCharTok{$}\NormalTok{test.b,}
    \AttributeTok{condicao =}\NormalTok{ dados}\SpecialCharTok{$}\NormalTok{Condition}
\NormalTok{)}

\NormalTok{amostra\_teste\_t\_RQ3\_1\_controle }\OtherTok{=}
\NormalTok{  dados\_teste\_t\_perm\_RQ3}\SpecialCharTok{$}\NormalTok{score\_teste\_experimental\_controlado[}
\NormalTok{    dados\_teste\_t\_perm\_RQ3}\SpecialCharTok{$}\NormalTok{condicao }\SpecialCharTok{==} \StringTok{"Control"}
\NormalTok{  ]}

\NormalTok{amostra\_teste\_t\_RQ3\_1\_experimental }\OtherTok{=}
\NormalTok{  dados\_teste\_t\_perm\_RQ3}\SpecialCharTok{$}\NormalTok{score\_teste\_experimental\_controlado[}
\NormalTok{    dados\_teste\_t\_perm\_RQ3}\SpecialCharTok{$}\NormalTok{condicao }\SpecialCharTok{==} \StringTok{"Full"}
\NormalTok{  ]}
...
\CommentTok{\# Mas será que as amostras passaram no teste de normalidade?}

\CommentTok{\# Não passou no teste de normalidade}
\FunctionTok{shapiro.test}\NormalTok{(dados}\SpecialCharTok{$}\NormalTok{test.b)}
\end{Highlighting}
\end{Shaded}
...
## 
## data:  dados$test.b
## W = 0.9492, p-value = 0.00139
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Não passou no teste de normalidade}
\FunctionTok{shapiro.test}\NormalTok{(dados}\SpecialCharTok{$}\NormalTok{test.i)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\# Teste de hipótese de duas amostras não paramétrico}
\CommentTok{\# Como as amostras não passaram na premissa de normalidade,}
\CommentTok{\# utilizaremos o método não paramétrico de permutação}

\CommentTok{\# H0: mu(Prática intercalada) = mu(Prática bloqueada)}
\CommentTok{\# Ha: mu(Prática bloqueada) != mu(Prática intercalada)}

\NormalTok{n }\OtherTok{=} \FunctionTok{length}\NormalTok{(dados}\SpecialCharTok{$}\NormalTok{test.i)}
\NormalTok{medidas\_RQ3 }\OtherTok{=} \FunctionTok{c}\NormalTok{(dados}\SpecialCharTok{$}\NormalTok{test.i, dados}\SpecialCharTok{$}\NormalTok{test.b)}
\NormalTok{grupos\_RQ3 }\OtherTok{=} \FunctionTok{c}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\StringTok{"Prática intercalada"}\NormalTok{, n), }\FunctionTok{rep}\NormalTok{(}\StringTok{"Prática bloqueada"}\NormalTok{, n))}

\CommentTok{\# Diferença observada entre as médias}
\NormalTok{(}\AttributeTok{delta\_medias\_obs\_RQ3 =} \FunctionTok{abs}\NormalTok{(}\FunctionTok{mean}\NormalTok{(}
\NormalTok{    medidas\_RQ3[grupos\_RQ3}\SpecialCharTok{==}\StringTok{"Prática intercalada"}\NormalTok{]) }\SpecialCharTok{{-}} \FunctionTok{mean}\NormalTok{(}
\NormalTok{        medidas\_RQ3[grupos\_RQ3}\SpecialCharTok{==}\StringTok{"Prática bloqueada"}\NormalTok{])))}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{Projeto_em_grupo_versao-3--pdf-_files/figure-latex/unnamed-chunk-20-1.pdf}
\includegraphics{Projeto_em_grupo_versao-3--pdf-_files/figure-latex/unnamed-chunk-20-2.pdf}
...
\begin{figure}
\centering
\includegraphics[width=12cm,height=\textheight]{"G:/Meu Drive/UFABC/BC&T/8º quad/Bioestatística/bioestatistica_projeto_final/arquivos_importantes_projeto_dados_imagens/Imagens/grafico_RQ3.png"}
...
\includegraphics{Projeto_em_grupo_versao-3--pdf-_files/figure-latex/unnamed-chunk-21-1.pdf}
